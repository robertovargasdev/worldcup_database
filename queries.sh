...
echo "$($PSQL "SELECT name FROM teams INNER JOIN games ON ( team_id = winner_id OR team_id = opponent_id)  WHERE round = 'Eighth-Final' AND year = 2014 ORDER BY name;")"
...
echo "$($PSQL "SELECT name FROM teams INNER JOIN games ON team_id = winner_id WHERE name LIKE 'Co%' ORDER BY name;")"
